...
if [[ "$USE_JAEGER" = true ]] ; then
    docker-compose -f docker-compose-atm.yml up $DOCKER_ARGS jaeger
    sleep 3
...
if [[ "$M3DBNODE_DEV_IMG" == "0" ]] || [[ "$FORCE_BUILD" == true ]] || [[ "$BUILD_M3DBNODE" == true ]]; then
    ...
    docker-compose -f docker-compose-atm.yml up --build $DOCKER_ARGS m3db_seed
else
    docker-compose -f docker-compose-atm.yml up $DOCKER_ARGS m3db_seed
fi

# Bring up any other replicas
if [[ "$USE_MULTI_DB_NODES" = true ]] ; then
    echo "Running multi node"
    docker-compose -f docker-compose-atm.yml up $DOCKER_ARGS m3db_data01
    docker-compose -f docker-compose-atm.yml up $DOCKER_ARGS m3db_data02
else
...
if [[ "$M3COORDINATOR_DEV_IMG" == "0" ]] || [[ "$FORCE_BUILD" == true ]] || [[ "$BUILD_M3COORDINATOR" == true ]]; then
    ...
    docker-compose -f docker-compose-atm.yml up --build $DOCKER_ARGS m3coordinator01
else
    docker-compose -f docker-compose-atm.yml up $DOCKER_ARGS m3coordinator01
fi
...
if [[ "$USE_AGGREGATOR" = true ]]; then
    ...
    if [[ "$M3AGGREGATOR_DEV_IMG" == "0" ]] || [[ "$FORCE_BUILD" == true ]] || [[ "$BUILD_M3AGGREGATOR" == true ]]; then
        ...
        docker-compose -f docker-compose-atm.yml up --build $DOCKER_ARGS m3aggregator01
    else
        docker-compose -f docker-compose-atm.yml up $DOCKER_ARGS m3aggregator01
    fi

    if [[ "$USE_AGGREGATOR_HA" == true ]]; then
        # Bring up the second replica
        docker-compose -f docker-compose-atm.yml up $DOCKER_ARGS m3aggregator02
    fi
    ...
    if [[ "$M3COLLECTOR_DEV_IMG" == "0" ]] || [[ "$FORCE_BUILD" == true ]] || [[ "$BUILD_M3COLLECTOR" == true ]]; then
        ...
        docker-compose -f docker-compose-atm.yml up --build $DOCKER_ARGS m3collector01
    else
        docker-compose -f docker-compose-atm.yml up $DOCKER_ARGS m3collector01
    fi
else
...
if [[ "$USE_AGGREGATOR" = true ]]; then
    ...
    docker-compose -f docker-compose-atm.yml stop m3coordinator01
